var Syntax = {
    ...
    all: function () {
        ...
        for (var i = 0; i < all.length; i++) {
            var node = all[i];
            if (node.getAttribute("data-syntax")) {
                todo.push(node);
            }
        }
        while (todo.length) {
            this.apply(todo.shift());
        }
    },
    ...
    _process: function (node, syntax) {
        ...
        for (var i = 0; i < node.childNodes.length; i++) {
            code += node.childNodes[i].nodeValue || "";
        }
        ...
        if (node.outerHTML) { /* IE hack; innerHTML normalizes whitespace */
            ...
            while (tmp.firstChild.firstChild) {
                node.appendChild(tmp.firstChild.firstChild);
            }
